...
string = '''script 코드 head에다가 쓰기
    defer태그 쓰면 됨 ㅇㅇ
    <script src="script.js" defer></script>

유용한 명령어들
    alert():알림창에 괄호안의 메시지 띄우기
    console.log():콘솔에 괄호안의 메시지 띄우기

convention
    변수명:변수,함수이름은 소문자로 작성하고 이후나오는 단어는 대문자(camelCase)
        getNickName
    주석 적절히 잘 사용
    문장 끝날 때마다 세미콜론 추가
    적절한 들여쓰기
...
자료형
    ...
    typeof():자료형 반환
    number+string가능!
    템플릿 리터럴:f string같은거임
        `오늘 사과값은 ${price}원 입니다`
    null:변수값이 선언되지 않음
    undefined:변수값 선언되었지만 할당되진 않음
...
콘텐츠 조작
    element.textContent: 텍스트 조회,수정
        const winter = document.getElementsByClassName("winter");
        console.log(winter[0].textContent);
    element.innerHTML: 텍스트 뿐 아니라 Html 콘텐츠 수정,조회 가능
        const shirt=document.getElementsByTagName("li");
        shirt[0].innerHTML = "<strong>Yeah</strong>원"
        console.log(shirt[0].textContent)
...
def string_To_dic(test_string):
    ...
    #title 정함
    title = test_string.split('\n')[0]
    title = title.replace('<','')
    title = title.replace('>','')

    #string값을 string_lst에 넣음
    test_string = test_string.replace(title + '\n','')
    test_string_lst = test_string.split('\n')

    #string_lst 값을 content리스트에 넣음
    chunck = []
    for i in test_string_lst:
        if ('    ' in i and '        ' not in i):
            keys.append(i.strip())
            contents.append(chunck)
            chunck = []
        else:
            if ('        ' in i and '            ' not in i):
                i = i.replace('        ','&emsp;')
            else:
                i = i.replace('            ','&emsp;&emsp;')
            chunck.append(i)
    contents.append(chunck)
    contents.pop(0)

    #content리스트 안에 <br>끼워넣음


    #결과 값 딕셔너리에 넣음
    for i in range(len(keys)):
        result_dict[keys[i]] = contents[i]

    #결과값 출력
    print('<div class="box selector">')
    print(f'    <h1>{title}</h1>')
    print(f'    <hr>')
    print(f'    <ol>')
    for i in result_dict:
        ...
        for q in contents:
            print(space+'    ',end='')
            print(q,end='')
            if contents.index(q) != len(contents)-1:
                print('<br>')
            else:
                print()
        print(space+'</li>')
